...
final class CarPlaySceneDelegate: UIResponder, CPTemplateApplicationSceneDelegate {
    ...
    private func showLoadingTemplate() {
        guard let interfaceController else { return }
        let template = CPListTemplate(title: "Strimr", sections: [])
    ...
    private func showSignInPrompt() {
        ...
        let item = CPListItem(text: "Open Strimr on iPhone to sign in", detailText: nil)
        let section = CPListSection(items: [item])
        let template = CPListTemplate(title: "Strimr", sections: [section])
